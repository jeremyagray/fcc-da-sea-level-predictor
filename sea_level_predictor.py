...
def draw_plot():
    ...
    (long_slope, long_int, long_r, long_p, long_error) = linregress(
        x=df["year"], y=df["csiro"]
    )
    ...
    # 2050.
    (short_slope, short_int, short_r, short_p, short_error) = linregress(
        x=dfr["year"], y=dfr["csiro"]
    )
    ax.plot(
        years_recent,
